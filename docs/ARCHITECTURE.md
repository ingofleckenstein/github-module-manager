...
Nur konfigurierte Custom-Autoload-Pfade; Core bleibt gesperrt. Der Manager bleibt für normale Repository-Zuordnungen gesperrt und kann ausschließlich über eine dedizierte Einstellung konfiguriert werden. Diese erzeugt eine geschützte Zuordnung, deren Ziel zwingend der aktuell geladene Manager-Pfad sein muss; kein frei übergebener Zielpfad wird akzeptiert. Selbstupdates nutzen den normalen unveränderlichen-SHA-, Staging-, Sperr-, Rename-, Backup- und Journalablauf. Der laufende Request verwendet weiter die geladenen alten Klassen, führt den Dateitausch erst nach der vollständigen Vorprüfung aus und aktiviert den neuen Code beim nächsten Request. Pfade mit Symlinks werden abgewiesen. Module mit nicht statisch prüfbarer Konfiguration oder unbekannten Abhängigkeitsangaben werden im MVP konservativ abgewiesen, statt Anforderungen zu ignorieren. requirements.php wird erst nach expliziter Vertrauensbestätigung ausgeführt. Composer wird niemals gestartet.
